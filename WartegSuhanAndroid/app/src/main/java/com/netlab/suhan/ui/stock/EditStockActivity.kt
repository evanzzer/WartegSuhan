...
class EditStockActivity : AppCompatActivity() {
    ...
    private fun retrieveDelete(id: Int) {
        val responseListener = Response.Listener<String> { response ->
            if (response != null && response.isNotBlank()) {
                AlertDialog.Builder(this)
                    .setTitle("Informasi")
                    .setMessage("Data berhasil dihapus dari database")
        ...
        val request = WebRequest(Request.Method.POST, MenuActivity.URL + "/menu/" + id, responseListener, errorListener, HashMap())
        queue.add(request)
    }
    ...
    private fun retrieveUpdate(params: HashMap<String, String>) {
        ...
        val queue = Volley.newRequestQueue(this)
        val request = WebRequest(Request.Method.PUT, MenuActivity.URL + "/menu", responseListener, errorListener, params)
        queue.add(request)
    }
    ...
    private fun retrieveInsert(params: HashMap<String, String>) {
        val responseListener = Response.Listener<String> { response ->
            if (response != null && response.isNotBlank()) {
                AlertDialog.Builder(this)
                    .setTitle("Informasi")
                    .setMessage("Data berhasil dimasukkan ke dalam database")
                    .setPositiveButton("Kembali") { _, _ ->
                        setResult(RESULT_CODE)
                        finish()
                    }
        ...
        val queue = Volley.newRequestQueue(this)
        val request = WebRequest(Request.Method.POST, MenuActivity.URL + "/menu", responseListener, errorListener, params)
        queue.add(request)
    }
}